...
enum Client
{
	ClientId,
	ClientUserId,
	String:ClientAuthId[32],
	String:ClientName[64],
	String:ClientNameEscaped[128],
	ClientCredits,
	ClientOriginalCredits,
	ClientDateOfJoin,
	ClientDateOfLastJoin,
	ClientItems,
	ClientaEquipment[STORE_MAX_HANDLERS*STORE_MAX_SLOTS],
	ClientaEquipmentSynced[STORE_MAX_HANDLERS*STORE_MAX_SLOTS],
	Handle:ClienthCreditTimer,
	bool:ClientbLoaded
}

enum Menu_Handler
{
	String:MenuIdentifier[64],
	Handle:MenuhPlugin,
	Function:fnMenu,
	Function:fnHandler
}
...
public OnPluginStart()
{
	...
	{
		g_eClients[i][ClientCredits] = -1;
		g_eClients[i][ClientOriginalCredits] = 0;
		g_eClients[i][ClientItems] = -1;
		g_eClients[i][ClienthCreditTimer] = INVALID_HANDLE;
	}
...
public MenuHandler_GiveCredits(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		SetMenuTitle(m_hMenu, "Choose the amount of credits\n%N - %d credits", target, g_eClients[target][ClientCredits]);
		SetMenuExitBackButton(m_hMenu, true);
...
public Native_RegisterMenuHandler(Handle:plugin, numParams)
{
	...
	g_eMenuHandlers[m_iId][MenuhPlugin] = plugin;
	g_eMenuHandlers[m_iId][fnMenu] = GetNativeCell(2);
	g_eMenuHandlers[m_iId][fnHandler] = GetNativeCell(3);
	strcopy(g_eMenuHandlers[m_iId][MenuIdentifier], 64, m_szIdentifier);
...
{
	g_eItems[GetNativeCell(1)][Store_ItemiData] = GetNativeCell(2);
}

public Native_GetDataIndex(Handle:plugin, numParams)
{
	return g_eItems[GetNativeCell(1)][Store_ItemiData];
}
...
{
	return g_eClients[GetNativeCell(1)][ClientbLoaded];
}
...
{
	return g_eClients[GetNativeCell(1)][ClientCredits];
}
...
public Native_SetClientCredits(Handle:plugin, numParams)
{
	...
	Store_LogMessage(client, m_iCredits-g_eClients[client][ClientCredits], "Set by external plugin");
	g_eClients[client][ClientCredits] = m_iCredits;
	return 1;
}
...
public Native_IsItemInBoughtPackage(Handle:plugin, numParams)
{
	...
	decl m_iParent;
	if(itemid<0)
		m_iParent = g_eItems[itemid][Store_ItemiParent];
	else
		m_iParent = g_eItems[itemid][Store_ItemiParent];
		
	while(m_iParent != -1)
	{
		for(new i = 0; i<g_eClients[client][ClientItems]; ++i)
		{
			if(((uid == -1 && g_eClientItems[client][i][Client_ItemiUniqueId] == m_iParent) || (uid != -1 && g_eClientItems[client][i][Client_ItemiUniqueId] == uid)) && !g_eClientItems[client][i][Client_ItembDeleted])
				return true;
		}
		m_iParent = g_eItems[m_iParent][Store_ItemiParent];
	}
	return false;
}
...
public Native_GiveItem(Handle:plugin, numParams)
{
	...
	new m_iId = g_eClients[client][ClientItems]++;
	g_eClientItems[client][m_iId][Client_ItemiId] = -1;
	g_eClientItems[client][m_iId][Client_ItemiUniqueId] = itemid;
	g_eClientItems[client][m_iId][Client_ItemiDateOfPurchase] = m_iDateOfPurchase;
	g_eClientItems[client][m_iId][Client_ItemiDateOfExpiration] = m_iDateOfExpiration;
	g_eClientItems[client][m_iId][Client_ItemiPriceOfPurchase] = price;
...
public Native_RemoveItem(Handle:plugin, numParams)
{
	...
	if(m_iId != -1)
		g_eClientItems[client][m_iId][Client_ItembDeleted] = true;
}
...
public Native_GiveClientItem(Handle:plugin, numParams)
{
	...
	new m_iId = g_eClientItems[client][item][Client_ItemiUniqueId];
	new target = g_iMenuClient[client];
	g_eClientItems[client][item][Client_ItembDeleted] = true;
	Store_UnequipItem(client, m_iId);

	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItemiId] = -1;
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItemiUniqueId] = m_iId;
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItembSynced] = false;
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItembDeleted] = false;
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItemiDateOfPurchase] = g_eClientItems[target][item][Client_ItemiDateOfPurchase];
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItemiDateOfExpiration] = g_eClientItems[target][item][Client_ItemiDateOfExpiration];
	g_eClientItems[receiver][g_eClients[receiver][ClientItems]][Client_ItemiPriceOfPurchase] = g_eClientItems[target][item][Client_ItemiPriceOfPurchase];
	
	++g_eClients[receiver][ClientItems];
...
public Native_HasClientItem(Handle:plugin, numParams)
{
	...
	// Can he even have it?	
	if(!GetClientPrivilege(client, g_eItems[itemid][Store_ItemiFlagBits]))
		return false;

	// Is the item free (available for everyone)?
	if(g_eItems[itemid][Store_ItemiPrice] <= 0 && g_eItems[itemid][Store_ItemiPlans]==0)
		return true;
		
	// Is the client a VIP therefore has access to all the items already?
	if(Store_IsClientVIP(client) && !g_eItems[itemid][Store_ItembIgnoreVIP])
		return true;
	...
	for(new i=0;i<g_eClients[client][ClientItems];++i)
	{
		if(g_eClientItems[client][i][Client_ItemiUniqueId] == itemid && !g_eClientItems[client][i][Client_ItembDeleted])
			if(g_eClientItems[client][i][Client_ItemiDateOfExpiration]==0 || (g_eClientItems[client][i][Client_ItemiDateOfExpiration] && GetTime()<g_eClientItems[client][i][Client_ItemiDateOfExpiration]))
				return true;
			else
				return false;
	}
...
public Native_IterateEquippedItems(Handle:plugin, numParams)
{
	...
	{
		if(g_eClients[client][ClientaEquipment][i] >= 0 && (attributes==false || (attributes && g_eItems[g_eClients[client][ClientaEquipment][i]][Store_ItemhAttributes]!=INVALID_HANDLE)))
		{
			SetNativeCellRef(2, i);
			return g_eClients[client][ClientaEquipment][i];
		}
	}
...
{
	g_iSpam[client] = 0;
	g_eClients[client][ClientUserId] = GetClientUserId(client);
	g_eClients[client][ClientCredits] = -1;
	g_eClients[client][ClientOriginalCredits] = 0;
	g_eClients[client][ClientItems] = -1;
	g_eClients[client][ClientbLoaded] = false;
	for(new i=0;i<STORE_MAX_HANDLERS;++i)
	{
		for(new a=0;a<STORE_MAX_SLOTS;++a)
		{
			g_eClients[client][ClientaEquipment][i*STORE_MAX_SLOTS+a] = -2;
			g_eClients[client][ClientaEquipmentSynced][i*STORE_MAX_SLOTS+a] = -2;
		}
	}
...
{
	GetClientName(client, g_eClients[client][ClientName], 64);
	if(g_hDatabase)
		SQL_EscapeString(g_hDatabase, g_eClients[client][ClientName], g_eClients[client][ClientNameEscaped], 128);
}
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		g_eClients[attacker][ClientCredits] += GetMultipliedCredits(attacker, g_eCvars[g_cvarCreditAmountKill][aCache]);
		if(g_eCvars[g_cvarCreditMessages][aCache])
			Chat(attacker, "%t", "Credits Earned For Killing", g_eCvars[g_cvarCreditAmountKill][aCache], g_eClients[victim][ClientName]);
...
public Action:Command_Say(client, const String:command[], argc)
{
	if(argc > 0)
	{
		...
		if(m_szArg[0] == PublicChatTrigger)
		{
			...
			{
				if(strcmp(g_eItems[i][Store_ItemShortcut], m_szArg[1]) == 0 && g_eItems[i][Store_ItemShortcut][0] != 0)
				{
...
public Action:Command_Store(client, params)
{
	...
	if((g_eClients[client][ClientCredits] == -1 && g_eClients[client][ClientItems] == -1) || !g_eClients[client][ClientbLoaded])
	{
...
public Action:Command_Gift(client, params)
{
	...
	if(g_eClients[client][ClientCredits]<m_iCredits || m_iCredits<=0)
	{
	...
	g_eClients[client][ClientCredits] -= m_iCredits;
	g_eClients[m_iReceiver][ClientCredits] += m_iCredits;
	
	Chat(client, "%t", "Credit Gift Sent", m_iCredits, g_eClients[m_iReceiver][ClientName]);
	Chat(m_iReceiver, "%t", "Credit Gift Received", m_iCredits, g_eClients[client][ClientName]);
...
public Action:Command_GiveCredits(client, params)
{
	...
	if(m_iReceiver != -1)
	{
		...
		{
			if(client)
				Chat(client, "%t", "Credits Given", g_eClients[m_iReceiver][ClientName], m_iCredits);
			else
				ReplyToCommand(client, "%t", "Credits Given", g_eClients[m_iReceiver][ClientName], m_iCredits);
			Chat(m_iReceiver, "%t", "Credits Given", g_eClients[m_iReceiver][ClientName], m_iCredits);
		}
		else if(g_eCvars[g_cvarSilent][aCache] == 0)
			ChatAll("%t", "Credits Given", g_eClients[m_iReceiver][ClientName], m_iCredits);
...
public Action:Command_ResetPlayer(client, params)
{
	...
	if(strncmp(m_szTmp, "STEAM_", 6)==0)
	{
		...
		if(m_iReceiver == 0)
		{
			...
			SQL_TQuery(g_hDatabase, SQLCallback_ResetPlayer, m_szQuery, g_eClients[client][ClientUserId]);
		}
	}
	else
	{	
	...
	if(m_iReceiver != -1)
	{
		Store_LogMessage(client, -g_eClients[m_iReceiver][ClientCredits], "Player resetted");
		g_eClients[m_iReceiver][ClientCredits] = 0;
		for(new i=0;i<g_eClients[m_iReceiver][ClientItems];++i)
			Store_RemoveItem(m_iReceiver, g_eClientItems[m_iReceiver][i][Client_ItemiUniqueId]);
		ChatAll("%t", "Player Resetted", g_eClients[m_iReceiver][ClientName]);
	}
...
{	
	if(g_eClients[client][ClientCredits] == -1 && g_eClients[client][ClientItems] == -1)
	{
		Chat(client, "%t", "Inventory hasnt been fetched");
		return Plugin_Handled;
	}
	...
	{
		ChatAll("%t", "Player Credits", g_eClients[client][ClientName], g_eClients[client][ClientCredits]);
...
DisplayStoreMenu(client, parent=-1, last=-1)
{
	...
	if(parent!=-1)
	{
		SetMenuExitBackButton(m_hMenu, true);
		if(client == target)
			SetMenuTitle(m_hMenu, "%s\n%t", g_eItems[parent][ClientName], "Title Credits", g_eClients[target][ClientCredits]);
		else
			SetMenuTitle(m_hMenu, "%N\n%s\n%t", target, g_eItems[parent][ClientName], "Title Credits", g_eClients[target][ClientCredits]);
		g_iMenuBack[client] = g_eItems[parent][Store_ItemiParent];
	}
	else if(client == target)
		SetMenuTitle(m_hMenu, "%t\n%t", "Title Store", "Title Credits", g_eClients[target][ClientCredits]);
	else
		SetMenuTitle(m_hMenu, "%N\n%t\n%t", target, "Title Store", "Title Credits", g_eClients[target][ClientCredits]);
	...
	if(parent != -1)
	{
		if(g_eItems[parent][Store_ItemiPrice]>0)
		{
			if(!Store_IsClientVIP(target) && !Store_IsItemInBoughtPackage(target, parent))
			{
				if(g_eCvars[g_cvarSellEnabled][aCache])
				{
					AddMenuItemEx(m_hMenu, ITEMDRAW_DEFAULT, "sell_package", "%t", "Package Sell", RoundToFloor(g_eItems[parent][Store_ItemiPrice]*Float:g_eCvars[g_cvarSellRatio][aCache]));
					++m_iPosition;
				}
				...
				{
					if(g_eMenuHandlers[i][MenuhPlugin] == INVALID_HANDLE)
						continue;
					Call_StartFunction(g_eMenuHandlers[i][MenuhPlugin], g_eMenuHandlers[i][fnMenu]);
	...
	for(new i=0;i<g_iItems;++i)
	{
		if(g_eItems[i][Store_ItemiParent]==parent && (g_eCvars[g_cvarShowVIP][aCache] == 0 && GetClientPrivilege(target, g_eItems[i][Store_ItemiFlagBits], m_iFlags) || g_eCvars[g_cvarShowVIP][aCache]))
		{
			new m_iPrice = Store_GetLowestPrice(i);

			// This is a package
			if(g_eItems[i][Store_ItemiHandler] == g_iPackageHandler)
			{
				if(!Store_PackageHasClientItem(target, i, g_bInvMode[client]))
					continue;

				new m_iStyle = ITEMDRAW_DEFAULT;
				if(g_eCvars[g_cvarShowVIP][aCache] && !GetClientPrivilege(target, g_eItems[i][Store_ItemiFlagBits], m_iFlags))
					m_iStyle = ITEMDRAW_DISABLED;
				
				IntToString(i, STRING(m_szId));
				if(g_eItems[i][Store_ItemiPrice] == -1 || Store_HasClientItem(target, i))
					AddMenuItem(m_hMenu, m_szId, g_eItems[i][ClientName], m_iStyle);
				else if(!g_bInvMode[client] && g_eItems[i][Store_ItemiPlans]==0 && g_eItems[i][Store_ItembBuyable])
					InsertMenuItemEx(m_hMenu, m_iPosition, (m_iPrice<=g_eClients[target][ClientCredits]?ITEMDRAW_DEFAULT:ITEMDRAW_DISABLED), m_szId, "%t", "Item Available", g_eItems[i][Store_ItemName], g_eItems[i][Store_ItemiPrice]);
				else if(!g_bInvMode[client])
					InsertMenuItemEx(m_hMenu, m_iPosition, (m_iPrice<=g_eClients[target][ClientCredits]?ITEMDRAW_DEFAULT:ITEMDRAW_DISABLED), m_szId, "%t", "Item Plan Available", g_eItems[i][Store_ItemName]);
				++m_iPosition;
			}
			...
			else
			{
				...
				if(Store_HasClientItem(target, i))
				{
					if(Store_IsEquipped(target, i))
						InsertMenuItemEx(m_hMenu, m_iPosition, ITEMDRAW_DEFAULT, m_szId, "%t", "Item Equipped", g_eItems[i][ClientName]);
					else
						InsertMenuItemEx(m_hMenu, m_iPosition, ITEMDRAW_DEFAULT, m_szId, "%t", "Item Bought", g_eItems[i][ClientName]);
				}
				else if(!g_bInvMode[client])
				{				
					new m_iStyle = ITEMDRAW_DEFAULT;
					if((g_eItems[i][Store_ItemiPlans]==0 && g_eClients[target][ClientCredits]<m_iPrice) || (g_eCvars[g_cvarShowVIP][aCache] && !GetClientPrivilege(target, g_eItems[i][Store_ItemiFlagBits], m_iFlags)))
						m_iStyle = ITEMDRAW_DISABLED;
					
					if(!g_eItems[i][Store_ItembBuyable])
						continue;

					if(g_eItems[i][Store_ItemiPlans]==0)
						AddMenuItemEx(m_hMenu, m_iStyle, m_szId, "%t", "Item Available", g_eItems[i][Store_ItemName], g_eItems[i][Store_ItemiPrice]);
					else
						AddMenuItemEx(m_hMenu, m_iStyle, m_szId, "%t", "Item Plan Available", g_eItems[i][Store_ItemName], g_eItems[i][Store_ItemiPrice]);
				}
			}
		}
	}
	...
	else if (action == MenuAction_Select)
	{
		...
		if(menu == INVALID_HANDLE)
		{
			if(param2 == 0)
			{
				...
				if(g_eItems[g_iSelectedItem[client]][Store_ItemiHandler] == g_iPackageHandler)
		...
		else
		{
			...
			if(strcmp(m_szId, "sell_package")==0)
			{
				if(g_eCvars[g_cvarConfirmation][aCache])
				{
					...
					Format(STRING(m_szTitle), "%t", "Confirm_Sell", g_eItems[g_iSelectedItem[client]][Store_ItemName], g_eTypeHandlers[g_eItems[g_iSelectedItem[client]][Store_ItemiHandler]][szType], RoundToFloor(g_eItems[g_iSelectedItem[client]][Store_ItemiPrice]*Float:g_eCvars[g_cvarSellRatio][aCache]));
			...
			else if(!(48 <= m_szId[0] <= 57))
			{
				...
				for(new i=0;i<g_iMenuHandlers;++i)
				{
					Call_StartFunction(g_eMenuHandlers[i][MenuhPlugin], g_eMenuHandlers[i][fnHandler]);
			...
			else
			{
				...
				g_iMenuBack[client]=g_eItems[m_iId][Store_ItemiParent];
				g_iSelectedItem[client] = m_iId;
				g_iSelectedPlan[client] = -1;
				
				if((g_eClients[target][ClientCredits]>=g_eItems[m_iId][Store_ItemiPrice] || g_eItems[m_iId][Store_ItemiPlans]>0 && g_eClients[target][ClientCredits]>=Store_GetLowestPrice(m_iId)) && !Store_HasClientItem(target, m_iId) && g_eItems[m_iId][Store_ItemiPrice] != -1)				{
					if(g_eItems[m_iId][Store_ItemiPlans] > 0)
					{
						...
						{
							decl String:m_szTitle[128];
							Format(STRING(m_szTitle), "%t", "Confirm_Buy", g_eItems[m_iId][Store_ItemName], g_eTypeHandlers[g_eItems[m_iId][Store_ItemiHandler]][szType]);
				...
				if(g_eItems[m_iId][Store_ItemiHandler] != g_iPackageHandler)
				{				
				...
				{			
					if(Store_HasClientItem(target, m_iId) || g_eItems[m_iId][Store_ItemiPrice] == -1)
						DisplayStoreMenu(client, m_iId);
					else
						DisplayStoreMenu(client, g_eItems[m_iId][Store_ItemiParent]);
				}
			}
		}
	}
...
{
	g_iMenuNum[client] = 1;
	g_iMenuBack[client] = g_eItems[itemid][Store_ItemiParent];
	new target = g_iMenuClient[client];
	...
	new idx = 0;
	if(m_bEquipped)
		idx = Format(STRING(m_szTitle), "%t\n%t", "Item Equipped", g_eItems[itemid][ClientName], "Title Credits", g_eClients[target][ClientCredits]);
	else
		idx = Format(STRING(m_szTitle), "%s\n%t", g_eItems[itemid][ClientName], "Title Credits", g_eClients[target][ClientCredits]);
	...
	if(g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][bEquipable])
		if(!m_bEquipped)
	...
	if(!Store_IsClientVIP(target) && !Store_IsItemInBoughtPackage(target, itemid))
	{
		...
		{
			new uid = Store_GetClientItemId(client, itemid);
			if(g_eClientItems[client][uid][Client_ItemiDateOfExpiration] != 0)
			{
				new m_iLength = g_eClientItems[client][uid][Client_ItemiDateOfExpiration]-g_eClientItems[client][uid][Client_ItemiDateOfPurchase];
				new m_iLeft = g_eClientItems[client][uid][Client_ItemiDateOfExpiration]-GetTime();
				if(m_iLeft < 0)
					m_iLeft = 0;
...
public DisplayPlanMenu(client, itemid)
{
	...
	SetMenuTitle(m_hMenu, "%s\n%t", g_eItems[itemid][Store_ItemName], "Title Credits", g_eClients[target][ClientCredits]);
...
public MenuHandler_Plan(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{
			decl String:m_szTitle[128];
			Format(STRING(m_szTitle), "%t", "Confirm_Buy", g_eItems[g_iSelectedItem[client]][Store_ItemName], g_eTypeHandlers[g_eItems[g_iSelectedItem[client]][Store_ItemiHandler]][szType]);
			Store_DisplayConfirmMenu(client, m_szTitle, MenuHandler_Store, 0);
			return;
		}
		else
		...
		else
		{
			...
			else if(m_iId == 1)
			{
				if(g_eCvars[g_cvarConfirmation][aCache])
				{
					...
					decl String:m_szTitle[128];
					Format(STRING(m_szTitle), "%t", "Confirm_Sell", g_eItems[g_iSelectedItem[client]][Store_ItemName], g_eTypeHandlers[g_eItems[g_iSelectedItem[client]][Store_ItemiHandler]][szType], m_iCredits);
					g_iMenuNum[client] = 2;
...
public DisplayPlayerMenu(client)
{
	...
	SetMenuExitBackButton(m_hMenu, true);
	SetMenuTitle(m_hMenu, "%t\n%t", "Title Gift", "Title Credits", g_eClients[client][ClientCredits]);
	...
	LoopIngamePlayers(i)
	{
		...
		if(!GetClientPrivilege(i, g_eItems[g_iSelectedItem[client]][Store_ItemiFlagBits], m_iFlags))
			continue;

		if(i != target && IsClientInGame(i) && !Store_HasClientItem(i, g_iSelectedItem[client]))
		{
			IntToString(g_eClients[i][ClientUserId], STRING(m_szID));
			AddMenuItem(m_hMenu, m_szID, g_eClients[i][ClientName]);
			++m_iCount;
		}
	}
...
public MenuHandler_Gift(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else
		{
			...
			{
				decl String:m_szTitle[128];
				Format(STRING(m_szTitle), "%t", "Confirm_Gift", g_eItems[g_iSelectedItem[client]][Store_ItemName], g_eTypeHandlers[g_eItems[g_iSelectedItem[client]][Store_ItemiHandler]][szType], g_eClients[m_iReceiver][ClientName]);
...
public ConVar_CreditTimer(index)
{
	...
	for(new i=1;i<=MaxClients;++i)
	{
		ClearTimer(g_eClients[i][ClienthCreditTimer]);
		if(m_bTimer && IsClientInGame(i))
			g_eClients[i][ClienthCreditTimer] = Store_CreditTimer(i);
	}
}
...
public SQLCallback_LoadClientInventory_Credits(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	else
	{
		...
		new m_iTime = GetTime();
		g_eClients[client][ClientUserId] = userid;
		g_eClients[client][ClientItems] = -1;
		GetLegacyAuthString(client, STRING(m_szSteamID), false);
		strcopy(g_eClients[client][ClientAuthId], 32, m_szSteamID[8]);
		GetClientName(client, g_eClients[client][ClientName], 64);
		SQL_EscapeString(g_hDatabase, g_eClients[client][ClientName], g_eClients[client][ClientNameEscaped], 128);
		
		if(SQL_FetchRow(hndl))
		{
			g_eClients[client][ClientId] = SQL_FetchInt(hndl, 0);
			g_eClients[client][ClientCredits] = SQL_FetchInt(hndl, 3);
			g_eClients[client][ClientOriginalCredits] = SQL_FetchInt(hndl, 3);
			g_eClients[client][ClientDateOfJoin] = SQL_FetchInt(hndl, 4);
			g_eClients[client][ClientDateOfLastJoin] = m_iTime;
			
			Format(STRING(m_szQuery), "SELECT * FROM store_items WHERE `player_id`=%d", g_eClients[client][ClientId]);
			SQL_TQuery(g_hDatabase, SQLCallback_LoadClientInventory_Items, m_szQuery, userid);

			Store_LogMessage(client, g_eClients[client][ClientCredits], "Amount of credits when the player joined");
		...
		else
		{
			Format(STRING(m_szQuery), "INSERT INTO store_players (`authid`, `name`, `credits`, `date_of_join`, `date_of_last_join`) VALUES(\"%s\", '%s', %d, %d, %d)",
						g_eClients[client][ClientAuthId], g_eClients[client][ClientNameEscaped], g_eCvars[g_cvarStartCredits][aCache], m_iTime, m_iTime);
			SQL_TQuery(g_hDatabase, SQLCallback_InsertClient, m_szQuery, userid);
			g_eClients[client][ClientCredits] = g_eCvars[g_cvarStartCredits][aCache];
			g_eClients[client][ClientOriginalCredits] = g_eCvars[g_cvarStartCredits][aCache];
			g_eClients[client][ClientDateOfJoin] = m_iTime;
			g_eClients[client][ClientDateOfLastJoin] = m_iTime;
			g_eClients[client][ClientbLoaded] = true;
			g_eClients[client][ClientItems] = 0;

			if(g_eCvars[g_cvarStartCredits][aCache] > 0)
				Store_LogMessage(client, g_eCvars[g_cvarStartCredits][aCache], "Start credits");
		}
		
		g_eClients[client][ClienthCreditTimer] = Store_CreditTimer(client);
	}
}
...
public SQLCallback_LoadClientInventory_Items(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	{	
		...
		decl String:m_szQuery[256];
		Format(STRING(m_szQuery), "SELECT * FROM store_equipment WHERE `player_id`=%d", g_eClients[client][ClientId]);
		...
		while(SQL_FetchRow(hndl))
		{
			...
			{
				g_eClientItems[client][i][Client_ItemiId] = SQL_FetchInt(hndl, 0);
				g_eClientItems[client][i][Client_ItemiUniqueId] = m_iUniqueId;
				g_eClientItems[client][i][Client_ItembSynced] = true;
				g_eClientItems[client][i][Client_ItembDeleted] = false;
				g_eClientItems[client][i][Client_ItemiDateOfPurchase] = SQL_FetchInt(hndl, 4);
				g_eClientItems[client][i][Client_ItemiDateOfExpiration] = m_iExpiration;
				g_eClientItems[client][i][Client_ItemiPriceOfPurchase] = SQL_FetchInt(hndl, 6);
			
				++i;
			}
		}
		g_eClients[client][ClientItems] = i;
	}
}
...
public SQLCallback_InsertClient(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	else
	{
		...
		g_eClients[client][ClientId] = SQL_GetInsertId(hndl);
	}
}
...
public SQLCallback_ReloadConfig(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	else
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			g_eItems[g_iItems][Store_ItemiId] = SQL_FetchInt(hndl, 0);
			g_eItems[g_iItems][Store_ItemiParent] = SQL_FetchInt(hndl, 1);
			g_eItems[g_iItems][Store_ItemiPrice] = SQL_FetchInt(hndl, 2);
			
			IntToString(g_eItems[g_iItems][Store_ItemiId], g_eItems[g_iItems][Store_ItemUniqueId], PLATFORM_MAX_PATH);
			...
			g_eItems[g_iItems][Store_ItemiHandler] = m_iHandler;
			
			SQL_FetchString(hndl, 4, STRING(m_szFlag));
			g_eItems[g_iItems][Store_ItemiFlagBits] = ReadFlagString(m_szFlag);
			
			SQL_FetchString(hndl, 5, g_eItems[g_iItems][Store_ItemName], ITEM_NAME_LENGTH);
...
public Store_LoadClientInventory(client)
{
	...
	SQL_TQuery(g_hDatabase, SQLCallback_LoadClientInventory_Credits, m_szQuery, g_eClients[client][ClientUserId]);
}
...
public Store_SaveClientInventory(client)
{
	...
	for(new i=0;i<g_eClients[client][ClientItems];++i)
	{
		strcopy(STRING(m_szType), g_eTypeHandlers[g_eItems[g_eClientItems[client][i][Client_ItemiUniqueId]][Store_ItemiHandler]][szType]);
		strcopy(STRING(m_szUniqueId), g_eItems[g_eClientItems[client][i][Client_ItemiUniqueId]][Store_ItemName]);
	
		if(!g_eClientItems[client][i][Client_ItembSynced] && !g_eClientItems[client][i][Client_ItembDeleted])
		{
			g_eClientItems[client][i][Client_ItembSynced] = true;
			Format(STRING(m_szQuery), "INSERT INTO store_items (`player_id`, `type`, `unique_id`, `date_of_purchase`, `date_of_expiration`, `price_of_purchase`) VALUES(%d, \"%s\", \"%s\", %d, %d, %d)", g_eClients[client][ClientId], m_szType, m_szUniqueId, g_eClientItems[client][i][Client_ItemiDateOfPurchase], g_eClientItems[client][i][Client_ItemiDateOfExpiration], g_eClientItems[client][i][Client_ItemiPriceOfPurchase]);
			SQL_TVoid(g_hDatabase, m_szQuery);
		} 

		else if(g_eClientItems[client][i][Client_ItembSynced] && g_eClientItems[client][i][Client_ItembDeleted])
		{
			// Might have been synced already but ID wasn't acquired
			if(g_eClientItems[client][i][ClientId]==-1)
				Format(STRING(m_szQuery), "DELETE FROM store_items WHERE `player_id`=%d AND `type`=\"%s\" AND `unique_id`=\"%s\"", g_eClients[client][ClientId], m_szType, m_szUniqueId);
			else
				Format(STRING(m_szQuery), "DELETE FROM store_items WHERE `id`=%d", g_eClientItems[client][i][ClientId]);
...
public Store_SaveClientEquipment(client)
{
	...
	for(new i=0;i<STORE_MAX_HANDLERS;++i)
	{
		for(new a=0;a<STORE_MAX_SLOTS;++a)
		{
			...
			if(g_eClients[client][ClientaEquipmentSynced][m_iId] == g_eClients[client][ClientaEquipment][m_iId])
				continue;
			else if(g_eClients[client][ClientaEquipmentSynced][m_iId] != -2)
				if(g_eClients[client][ClientaEquipment][m_iId]==-1)
					Format(STRING(m_szQuery), "DELETE FROM store_equipment WHERE `player_id`=%d AND `type`=\"%s\" AND `slot`=%d", g_eClients[client][ClientId], g_eTypeHandlers[i][szType], a);
				else
					Format(STRING(m_szQuery), "UPDATE store_equipment SET `unique_id`=\"%s\" WHERE `player_id`=%d AND `type`=\"%s\" AND `slot`=%d", g_eItems[g_eClients[client][ClientaEquipment][m_iId]][Store_ItemUniqueId], g_eClients[client][ClientId], g_eTypeHandlers[i][szType], a);
				
			else
				Format(STRING(m_szQuery), "INSERT INTO store_equipment (`player_id`, `type`, `unique_id`, `slot`) VALUES(%d, \"%s\", \"%s\", %d)", g_eClients[client][ClientId], g_eTypeHandlers[i][szType], g_eItems[g_eClients[client][ClientaEquipment][m_iId]][Store_ItemUniqueId], a);

			SQL_TVoid(g_hDatabase, m_szQuery);
			g_eClients[client][ClientaEquipmentSynced][m_iId] = g_eClients[client][ClientaEquipment][m_iId];
		}
	}
}
...
public Store_SaveClientData(client)
{
	...
	decl String:m_szQuery[256];
	if(g_bMySQL)
		Format(STRING(m_szQuery), "UPDATE store_players SET `credits`=GREATEST(`credits`+%d,0), `date_of_last_join`=%d, `name`='%s' WHERE `id`=%d", g_eClients[client][ClientCredits]-g_eClients[client][ClientOriginalCredits], g_eClients[client][ClientDateOfLastJoin], g_eClients[client][ClientNameEscaped], g_eClients[client][ClientId]);
	else
		Format(STRING(m_szQuery), "UPDATE store_players SET `credits`=MAX(`credits`+%d,0), `date_of_last_join`=%d, `name`='%s' WHERE `id`=%d", g_eClients[client][ClientCredits]-g_eClients[client][ClientOriginalCredits], g_eClients[client][ClientDateOfLastJoin], g_eClients[client][ClientNameEscaped], g_eClients[client][ClientId]);

	g_eClients[client][ClientOriginalCredits] = g_eClients[client][ClientCredits];
...
{
	Store_LogMessage(client, g_eClients[client][ClientCredits], "Amount of credits when the player left");
	g_eClients[client][ClientCredits] = -1;
	g_eClients[client][ClientOriginalCredits] = -1;
	g_eClients[client][ClientItems] = -1;
	g_eClients[client][ClientbLoaded] = false;
	ClearTimer(g_eClients[client][ClienthCreditTimer]);
}

Store_GetItemId(String:type[], String:uid[], start=-1)
{
	for(new i=start+1;i<g_iItems;++i)
		if(strcmp(g_eTypeHandlers[g_eItems[i][Store_ItemiHandler]][szType], type)==0 && strcmp(g_eItems[i][Store_ItemUniqueId], uid)==0 && g_eItems[i][Store_ItemiPrice] >= 0)
			return i;
	return -1;
}
...
Store_BuyItem(client, itemid, plan=-1)
{
	...
	g_eClients[client][ClientCredits] -= m_iPrice;

	Store_LogMessage(client, -g_eItems[itemid][Store_ItemiPrice], "Bought a %s %s", g_eItems[itemid][Store_ItemName], g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][szType]);
	
	Chat(client, "%t", "Chat Bought Item", g_eItems[itemid][Store_ItemName], g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][szType]);
}
...
{	
	...
	g_eClients[client][ClientCredits] += m_iCredits;
	Chat(client, "%t", "Chat Sold Item", g_eItems[itemid][Store_ItemName], g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][szType]);
	
	Store_LogMessage(client, m_iCredits, "Sold a %s %s", g_eItems[itemid][Store_ItemName], g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][szType]);
...
public Store_GiftItem(client, receiver, item)
{
	...
	Chat(client, "%t", "Chat Gift Item Sent", g_eClients[receiver][ClientName], g_eItems[m_iId][Store_ItemName], g_eTypeHandlers[g_eItems[m_iId][Store_ItemiHandler]][szType]);
	Chat(receiver, "%t", "Chat Gift Item Received", g_eClients[target][ClientName], g_eItems[m_iId][Store_ItemName], g_eTypeHandlers[g_eItems[m_iId][Store_ItemiHandler]][szType]);

	Store_LogMessage(client, 0, "Gifted a %s to %N", g_eItems[m_iId][ClientName], receiver);
}
...
{
	return CreateTimer(g_eCvars[g_cvarCreditTimer][aCache], Timer_CreditTimer, g_eClients[client][ClientUserId], TIMER_REPEAT);
}
...
Store_WalkConfig(&Handle:kv, parent=-1)
{
	...
	do
	{
		...
		{
			KvGoBack(kv);
			KvGetSectionName(kv, g_eItems[g_iItems][Store_ItemName], 64);
			KvGetSectionName(kv, g_eItems[g_iItems][Store_ItemUniqueId], 64);
			ReplaceString(g_eItems[g_iItems][Store_ItemName], 64, "\\n", "\n");
			KvGetString(kv, "shortcut", g_eItems[g_iItems][Store_ItemShortcut], 64);
			...
			if(m_szGame[0] != 0 && StrContains(m_szGame, g_szGameDir)==-1)
				continue;
			g_eItems[g_iItems][Store_ItemiFlagBits] = ReadFlagString(m_szFlags);
			g_eItems[g_iItems][Store_ItemiPrice] = KvGetNum(kv, "price", -1);
			g_eItems[g_iItems][Store_ItembBuyable] = (KvGetNum(kv, "buyable", 1)?true:false);
			g_eItems[g_iItems][Store_ItembIgnoreVIP] = (KvGetNum(kv, "ignore_vip", 0)?true:false);
			g_eItems[g_iItems][Store_ItemiHandler] = g_iPackageHandler;
			KvGotoFirstSubKey(kv);
			
			g_eItems[g_iItems][Store_ItemiParent] = parent;
		...
		else
		{
			...
			g_eItems[g_iItems][Store_ItemiParent] = parent;
			KvGetSectionName(kv, g_eItems[g_iItems][Store_ItemName], ITEM_NAME_LENGTH);
			g_eItems[g_iItems][Store_ItemiPrice] = KvGetNum(kv, "price");
			g_eItems[g_iItems][Store_ItembBuyable] = KvGetNum(kv, "buyable", 1)?true:false;
			g_eItems[g_iItems][Store_ItembIgnoreVIP] = (KvGetNum(kv, "ignore_vip", 0)?true:false);
			...
			if(KvGetNum(kv, "unique_id", -1)==-1)
				KvGetString(kv, g_eTypeHandlers[m_iHandler][szUniqueKey], g_eItems[g_iItems][Store_ItemUniqueId], PLATFORM_MAX_PATH);
			else
				KvGetString(kv, "unique_id", g_eItems[g_iItems][Store_ItemUniqueId], PLATFORM_MAX_PATH);
			...
			if(KvJumpToKey(kv, "Plans"))
			{
				...
				g_eItems[g_iItems][Store_ItemiPlans]=index;

				KvGoBack(kv);
				KvGoBack(kv);
			}

			if(g_eItems[g_iItems][Store_ItemhAttributes])
				CloseHandle(g_eItems[g_iItems][Store_ItemhAttributes]);

			g_eItems[g_iItems][Store_ItemhAttributes] = INVALID_HANDLE;
			if(KvJumpToKey(kv, "Attributes"))
			{
				g_eItems[g_iItems][Store_ItemhAttributes] = CreateTrie();
				...
				do
				{
					...
					SetTrieString(g_eItems[g_iItems][Store_ItemhAttributes], m_szAttribute, m_szValue);
...
public Store_GetMenuHandler(String:id[])
{
	for(new i=0;i<g_iMenuHandlers;++i)
	{
		if(strcmp(g_eMenuHandlers[i][MenuIdentifier], id)==0)
			return i;
	}
	return -1;
}
...
public bool:Store_IsEquipped(client, itemid)
{
	for(new i=0;i<STORE_MAX_SLOTS;++i)
		if(g_eClients[client][ClientaEquipment][g_eItems[itemid][Store_ItemiHandler]*STORE_MAX_SLOTS+i] == itemid)
			return true;
	return false;
}
...
public Store_GetExpiration(client, itemid)
{
	...
	if(uid<0)
		return 0;
	return g_eClientItems[client][uid][Client_ItemiDateOfExpiration];
}
...
Store_UseItem(client, itemid, bool:synced=false, slot=0)
{
	...
	if(g_eTypeHandlers[g_eItems[itemid][Store_ItemiHandler]][bEquipable])
	{
		g_eClients[client][ClientaEquipment][g_eItems[itemid][Store_ItemiHandler]*STORE_MAX_SLOTS+m_iSlot]=itemid;
		if(synced)
			g_eClients[client][ClientaEquipmentSynced][g_eItems[itemid][Store_ItemiHandler]*STORE_MAX_SLOTS+m_iSlot]=itemid;
	}
	else if(m_iSlot == 0)
	{
		Store_RemoveItem(client, itemid);
...
Store_UnequipItem(client, itemid, bool:fn=true)
{
	...
	decl m_iId;
	if(g_eItems[itemid][Store_ItemiHandler] != g_iPackageHandler)
	{
		m_iId = g_eItems[itemid][Store_ItemiHandler]*STORE_MAX_SLOTS+m_iSlot;
		if(g_eClients[client][ClientaEquipmentSynced][m_iId]==-2)
			g_eClients[client][ClientaEquipment][m_iId]=-2;
		else
			g_eClients[client][ClientaEquipment][m_iId]=-1;
	}
	else
	{
		for(new i=0;i<STORE_MAX_HANDLERS;++i)
		{
			...
			{
				if(g_eClients[client][ClientaEquipment][i+a] < 0)
					continue;
				m_iId = i*STORE_MAX_SLOTS+a;
				if(Store_IsItemInBoughtPackage(client, g_eClients[client][ClientaEquipment][m_iId], itemid))
					if(g_eClients[client][ClientaEquipmentSynced][m_iId]==-2)
						g_eClients[client][ClientaEquipment][m_iId]=-2;
					else
						g_eClients[client][ClientaEquipment][m_iId]=-1;
...
{
	return g_eClients[client][ClientaEquipment][handler*STORE_MAX_SLOTS+slot];
}

Store_PackageHasClientItem(client, packageid, bool:invmode=false)
{
	new m_iFlags = GetUserFlagBits(client);
	if(!g_eCvars[g_cvarShowVIP][aCache] && !GetClientPrivilege(client, g_eItems[packageid][Store_ItemiFlagBits], m_iFlags))
		return false;
	for(new i=0;i<g_iItems;++i)
		if(g_eItems[i][Store_ItemiParent] == packageid && (g_eCvars[g_cvarShowVIP][aCache] || GetClientPrivilege(client, g_eItems[i][Store_ItemiFlagBits], m_iFlags)) && (invmode && Store_HasClientItem(client, i) || !invmode))
			if((g_eItems[i][Store_ItemiHandler] == g_iPackageHandler && Store_PackageHasClientItem(client, i, invmode)) || g_eItems[i][Store_ItemiHandler] != g_iPackageHandler)
				return true;
	return false;
}
...
Store_LogMessage(client, credits, const String:message[], ...)
{
	...
	{
		decl String:m_szQuery[256];
		Format(STRING(m_szQuery), "INSERT INTO store_logs (player_id, credits, reason, date) VALUES(%d, %d, \"%s\", %d)", g_eClients[client][ClientId], credits, m_szReason, GetTime());
		SQL_TVoid(g_hDatabase, m_szQuery);
	}
}
...
Store_GetClientItemPrice(client, itemid)
{
	...
	if(g_eClientItems[client][uid][Client_ItemiPriceOfPurchase]==0)
		return g_eItems[itemid][Store_ItemiPrice];

	return g_eClientItems[client][uid][Client_ItemiPriceOfPurchase];
}
