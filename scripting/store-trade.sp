...
public DisplayTradeMenu(client)
{
	...
	for(new i=0;i<STORE_TRADE_MAX_OFFERS;++i)
	{
		...
		AddMenuItemEx(m_hMenu, ITEMDRAW_DEFAULT, m_szItemID, "%s %s", m_eItem[Store_ItemName], m_eHandler[szType]);
	}
...
public bool:Trade_OnHandler(client, String:info[], itemid)
{
	...
	if(strcmp(info, "add_to_offer")==0)
	{
		...
		Format(m_szTitle, sizeof(m_szTitle), "%t", "Confirm_Offer_Item", m_eItem[Store_ItemName], m_eHandler[szType]);
	...
	} else if(strcmp(info, "trade_this_item")==0)
	{
		...
		Store_GetHandler(m_eItem[Store_ItemiHandler], m_eHandler);
		decl String:m_szTitle[128];
		Format(m_szTitle, sizeof(m_szTitle), "%t", "Confirm_Trade_Item", m_eItem[Store_ItemName], m_eHandler[szType]);
		Store_SetClientMenu(client, 2);
		if(Store_ShouldConfirm())
